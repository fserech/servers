...
export class ServerListComponent implements OnInit, OnDestroy {
  ...
  get filtered(): Server[] {
    return this.servers.filter(s => {
      const typeFilters = ['database','web','api','infra','security','file','dns']; const ms = this.filterStatus==='all' || (this.filterStatus==='active'&&(s.status==='active'||s.status==='critical')) || (this.filterStatus==='inactive'&&(s.status==='inactive'||s.status==='maintenance')) || (typeFilters.includes(this.filterStatus) && s.type===this.filterStatus) || s.status===this.filterStatus;
  ...
  count(f: string) { if (f==='all') return this.servers.length; if (f==='active') return this.servers.filter(s=>s.status==='active'||s.status==='critical').length; if (f==='inactive') return this.servers.filter(s=>s.status==='inactive'||s.status==='maintenance').length; if (['database','web','api','infra','security','file','dns'].includes(f)) return this.servers.filter(s=>s.type===f).length; return this.servers.filter(s=>s.status===f).length; }
